...
## Phase 4：啟用每日排程與量測 🔄（排程已啟用，辨識期已結束、原主機先不停觀察中）

**目標**：正式上線。**依使用者決定，原自管主機先不停**（不是立即取代）——
持續觀察雲端版穩定性一段時間，累積足夠信心後再讓自管主機退場。

**已完成**：`schedule` cron 已啟用；真實寫入路徑已驗證（apply 建立 08-31 07:00–10:00
+ dry-run 回查「已存在」，證明寫入成功且冪等，見 [PDCA.md](PDCA.md)）；`keepalive.yml`
已上線並實測會自動 push（根治 60 天停用）；辨識期（07:00–10:00）已於 2026-09-03 結束，
時段改回真正離峰 00:30–06:00；同日排程觸發時間從 22:05 調整為 **20:30**，緩衝從
2h25m 拉高到 **4 小時**（實測延遲達 3.5–5.8 小時，超過原估計，見 PDCA.md）。
以下為原始規劃，保留供追溯：
...
4. **辨識期人工核對（已結束）**：兩邊並行期間，雲端版曾用 07:00–10:00 供人工在
   ...
   已改回離峰時段。
5. **舊系統退場（觀察中，尚未執行）**：待雲端版離峰時段運行足夠天數、可靠度數據
   充分後，停用原主機 crontab（先註解、一週後再刪），避免長期兩邊同時操作同一帳號。
   注意 `cleanup_candidates` 會清理所有過期的一次性預約（不分哪邊建的，保留最近一筆）。
6. 觀察 1–2 週：每隔幾天人工看一次 log（不能只看綠燈，見 design.md §8）；
   記錄 Actions 分鐘數用量與 reCAPTCHA 平均重試次數。

**完成條件**：連續 7 天以上由 GitHub Actions 成功維護隔日預約（離峰時段下）、
舊 crontab 已停用、延遲與耗時數據已記錄回 design.md §6 清單。
