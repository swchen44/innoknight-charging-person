...
## Phase 4：啟用每日排程與量測 🔄（排程已啟用，辨識期並行中）

**目標**：正式上線。**依使用者決定，原自管主機不停、兩邊並行**（不是立即取代）——
雲端版用辨識時段 07:00–10:00、自管主機維持 00:30–06:00，以時段區分供人工核對；
核對穩定後再改回離峰、讓自管主機退場。

**已完成**：`schedule` cron 已啟用（`5 14 * * *`）；真實寫入路徑已驗證（apply 建立
08-31 07:00–10:00 + dry-run 回查「已存在」，證明寫入成功且冪等，見 [PDCA.md](PDCA.md)）；
`keepalive.yml` 已上線並實測會自動 push（根治 60 天停用）。以下為原始規劃，保留供追溯：
...
4. **辨識期人工核對**：兩邊並行，雲端版用 07:00–10:00 供人工在 InnoKnight 上核對
   是否設定成功（不與自管主機的 00:30–06:00 混淆）。
5. **舊系統退場（核對穩定後）**：確認雲端版連續成功數天、且把時段改回離峰 00:30–06:00
   後，停用原主機 crontab（先註解、一週後再刪），避免長期兩邊同時操作同一帳號。
...
**完成條件**：連續 7 天以上由 GitHub Actions 成功維護隔日預約、時段改回離峰、
