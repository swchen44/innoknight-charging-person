...
## Phase 4：啟用每日排程與量測 ✅（已上線，舊系統已退場，持續觀察中）

**目標**：正式上線，取代原自管主機。

**狀態（2026-09-03）**：`schedule` cron 已啟用；真實寫入路徑已驗證（apply 建立
08-31 07:00–10:00 + dry-run 回查「已存在」，證明寫入成功且冪等，見 [PDCA.md](PDCA.md)）；
`keepalive.yml` 已上線並實測會自動 push（根治 60 天停用）；辨識期（07:00–10:00）已結束，
時段改回真正離峰 **00:20–06:00**（同日從 00:30 微調）；排程觸發時間從 22:05 先調整為
20:30，同日再提早到 **15:30**，緩衝從 2h25m 拉高到約 8h50m（實測延遲達 3.5–5.8 小時，
超過原估計，且要讓第二輪即使遇上同樣延遲也仍有恢復空間，見 PDCA.md）；**原自管主機已於
2026-09-03 停用**——早於原定「累積足夠穩定性數據」的完成條件，是使用者的決定，
不是本計劃的建議順序。**代價**：雲端版現在是唯一系統，任一晚失敗都沒有後備。
同日為此加了**第二輪排程備援**（16:00，第一輪 30 分鐘後）+ `concurrency.queue:
max`（避免兩輪在排隊延遲下互相取消，已實測驗證，見 PDCA.md），但新的雙輪觸發＋
離峰時段組合尚未經過真正的自動排程驗證（此前只驗證過手動 dispatch）；觸發時間
提早到下午也帶來新的裝置就緒風險。以下為原始規劃，保留供追溯：

1. workflow 加上 `schedule: - cron: "30 7 * * *"`（UTC，＝台北前一天 15:30），
   `target_date` 邏輯確認為「明天」。保留 `workflow_dispatch` 供手動補跑。
2. 先以 dry-run 模式跑 2–3 天，比對觸發時間戳量測**排程延遲分佈**。
3. 確認無誤後改為 `--apply` 正式模式。
4. **辨識期人工核對（已結束）**：兩邊並行期間，雲端版曾用 07:00–10:00 供人工在
   InnoKnight 上核對是否設定成功（不與自管主機的 00:30–06:00 混淆）；核對通過後
   已改回離峰時段。
5. **舊系統退場（已於 2026-09-03 執行）**：原主機 crontab 已停用。提前於原計劃
   （原定待穩定性數據充分才退場），需要提高近期人工巡查頻率補這個空缺
   （見 design.md §8、README「注意事項」）。
6. 觀察持續進行：每天人工看一次 log（不能只看綠燈，見 design.md §8）；
   記錄 Actions 分鐘數用量與 reCAPTCHA 平均重試次數。

**完成條件（原訂，作為持續觀察的參照基準，非阻斷條件）**：連續 7 天以上由
GitHub Actions 成功維護隔日預約（離峰時段下）、延遲與耗時數據已記錄回 design.md §6 清單。
...
- **無聲失效**：公開 repo 的 60 天停用已用 `keepalive.yml` 根治（每月自動 push）；
  設定錯誤類失敗會寄信；仍需定期人工看 log。
