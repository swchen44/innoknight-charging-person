...
時段改回真正離峰 00:30–06:00；排程觸發時間從 22:05 調整為 **20:30**，緩衝從 2h25m
拉高到 4 小時（實測延遲達 3.5–5.8 小時，超過原估計，見 PDCA.md）；**原自管主機已於
2026-09-03 停用**——早於原定「累積足夠穩定性數據」的完成條件，是使用者的決定，
不是本計劃的建議順序。**代價**：雲端版現在是唯一系統，任一晚失敗都沒有後備。
同日為此加了**第二輪排程備援**（21:00，第一輪 30 分鐘後）+ `concurrency.queue:
max`（避免兩輪在排隊延遲下互相取消，已實測驗證，見 PDCA.md），但新的雙輪觸發＋
離峰時段組合尚未經過真正的自動排程驗證（此前只驗證過手動 dispatch）。以下為
原始規劃，保留供追溯：

1. workflow 加上 `schedule: - cron: "30 12 * * *"`（UTC，＝台北前一晚 20:30），
