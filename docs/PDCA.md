...
**實作（初版）**：`daily-schedule.yml` 加第二個 `schedule` cron（UTC `0 13 * * *` = 台北
...
### 觸發時間各提早 5 小時、離峰預設改為 00:20（2026-09-03，同日再一次調整）

使用者接著指出：兩輪若緩衝都只有 4 小時／3.5 小時，一旦當晚遇上排程延遲上限
（實測過 5.8 小時），兩輪很可能被**同一波延遲一起壓縮**到窗口邊緣附近——第一輪
延遲晚到，第二輪（僅晚 30 分鐘名目差距）也大概率跟著延遲，實際執行時間可能相差
無幾，第二輪就失去「真正的恢復機會」這個存在意義。提議：兩輪的名目觸發時間都
**提早 5 小時**，讓即使兩輪都撞上同樣的延遲上限，實際執行時間仍離窗口夠遠。
同時要求充電時段「預設」改為 **00:20–06:00**（理解為 code 內建的 fallback 值也要
改，不只是 GitHub Variables，否則 Variables 被清空時仍會退回舊的 00:30）。

**實作**：

1. cron 第一輪 `30 12 * * *`→`30 7 * * *`（台北 20:30→15:30，緩衝 4h→約 8h50m）；
   第二輪 `0 13 * * *`→`0 8 * * *`（21:00→16:00，30 分鐘後不變，緩衝 3.5h→約 8h20m）。
2. 全部 code 層級的 `00:30` 預設改 `00:20`：`automation.py` 的 `AutomationConfig`、
   `scheduler.py` 的 `build_schedule_payload`/`has_equivalent_schedule` 兩處函式
   簽章、`main.py`/`browser_session.py` 的 argparse default、`.env.example`、
   workflow YAML 的 `inputs.start_time || vars.INNOKNIGHT_START_TIME || 'XX:XX'`
   fallback 鏈。測試同步更新（`test_scheduler.py` 的預設值斷言、
   `test_daily_workflow.py`/`test_browser_workflow.py` 的測試資料字面值），
   避免測試資料與生產預設不一致造成誤導。
3. GitHub Variables `INNOKNIGHT_START_TIME` 改為 `00:20`。

**驗證**（Run 33692753041，dry-run）：

```
Browser login OK: user_id=zWs8***
目標充電樁狀態: 充電樁已就緒
新增重點預約: *** 2026-09-04 00:20~06:00
Dry-run only; no schedule was changed.
```

正確算出新的離峰時段 00:20–06:00，GitHub 也正確辨識新的兩個 cron
（`30 7 * * *` / `0 8 * * *`，經 `gh workflow view --yaml` 確認）。

**權衡誠實記錄**：15:30 是台北時間下午，比先前任何一次調整（22:05→20:30）都
更早許多，對很多人的作息而言車輛可能還沒回家插上充電，會被 `device_not_ready`
正常跳過（綠燈但不建立預約）。這是本次改動用「緩衝」換來的代價，本文件與
design.md §4 第 1 條都已明確記錄，若近期常見這個情況需要重新評估。

### 首次真正自動排程驗證：15:30+16:00 雙輪 + 00:20 離峰（2026-09-03 晚間）

新組合（提早 5 小時的雙輪觸發、00:20 離峰時段、`queue: max`）第一次被真正的
`schedule` 事件（而非手動 dispatch）考驗，結果：

| 輪次 | 觸發（UTC） | 台北時間 | 延遲 | 登入 | 結果 |
|---|---|---|---|---|---|
| 第一輪（33753753718） | 12:10:56 | 20:10 | ~4h40m | 第 1 次即過 | 建立 09-04 00:20~06:00 |
| 第二輪（33755885542） | 12:33:22 | 20:33 | ~4h33m | 第 1 次被拒，第 2 次過 | 判定「已存在」，正確跳過 |

**觀察**：

1. **兩輪備援機制驗證成功**：第一輪成功後，第二輪即使自己也遇到 reCAPTCHA 拒絕，
   查詢後正確判定「已存在」並跳過，沒有重複建立——`has_equivalent_schedule` 冪等
   邏輯完全如預期運作，這是本專案第一次在真實排程情境下看到兩輪備援發揮作用
   （而非只是手動測試）。
2. **裝置就緒沒有出問題**：第一輪執行時「充電樁已就緒」，先前擔心的「15:30 太早、
   車還沒回家」風險這次沒有發生——但兩輪的實際延遲都在 4.5 小時上下，實際執行時間
   落在台北晚上 20:10/20:33（不是名目的下午 15:30/16:00），這個時段車輛已到家的
   機率本來就高，**這次的結果還不能斷定 15:30 這個名目時間點本身沒問題，需要更多
   樣本、特別是延遲較小、真的在下午執行的情況**。
3. **延遲幅度印證了「提早 5 小時」的必要性**：這次兩輪延遲幾乎相同（相差僅 7 分鐘，
   接近名目 30 分鐘差距），證實「兩輪會被同一波延遲一起影響」的假設。以這次
   4.5 小時左右的延遲幅度，若還是舊的 20:30/21:00 觸發，兩輪實際執行會落在台北
   凌晨 00:50 和 01:13，已經逼近甚至進入 00:20 窗口——調整前移的判斷是對的。

**待辦更新**：使用者決定接下來**持續觀察 3 天**，重點關注：兩輪是否每晚都準時
各自觸發、`device_not_ready` 出現頻率（判斷 15:30 是否對實際作息太早）、reCAPTCHA
重試分佈是否維持低頻率。
...
6. **兩輪排程備援（15:30 + 16:00）+ `concurrency.queue: max`**——原自管主機停用後
   雲端版無後備，加第二輪給 reCAPTCHA/裝置就緒狀態多一次機會；兩輪的名目觸發時間
   都提早 5 小時（原 20:30/21:00），確保即使兩輪都撞上實測過的延遲上限（5.8 小時），
   仍離窗口有足夠緩衝，第二輪才有意義。`queue: max` 是必要條件，否則預設的
   `queue: single` 會讓兩輪在特定延遲情境下互相取消（已實測驗證，見上）。
   代價是觸發時間落在台北下午，車輛可能還沒回家插上充電，見上方權衡記錄。
   第二輪不需要新的檢查邏輯——`has_equivalent_schedule`
...
- [x] **兩輪排程備援**：加第二輪 cron + `concurrency.queue: max`；
      手動連續 dispatch 實測確認兩輪確實依序排隊執行、不互相取消（見上）。
- [x] **觸發時間各提早 5 小時（15:30/16:00）+ 離峰預設改 00:20**：dry-run 驗證
      正確算出新時段，GitHub 正確辨識新 cron（見上「觸發時間各提早 5 小時」小節）。
- [x] **首次真正自動排程驗證（新組合：15:30+16:00 雙輪觸發 + 00:20 離峰時段 +
      零後備）**：2026-09-03 晚間兩輪皆自動觸發並成功，第二輪正確判定「已存在」
      並跳過，冪等機制在真實排程情境下驗證通過（見上「首次真正自動排程驗證」小節）。
- [~] **持續觀察 3 天（2026-09-03 起，進行中）**：使用者要求。重點：兩輪是否每晚
      準時各自觸發、`device_not_ready` 出現頻率（判斷 15:30 是否對實際作息太早，
      首夜未出現但延遲把實際執行時間推到晚上，樣本還不夠判斷名目下午時段本身）、
      reCAPTCHA 重試分佈是否維持低頻率、有無任何一晚兩輪都失敗（真正的漏建風險）。
