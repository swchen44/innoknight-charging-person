...
11. **第二輪排程備援（21:00，第一輪 30 分鐘後）+ `concurrency.queue: max`**——
    ...
    跑，以實測 3.5–5.8 小時延遲很可能發生）時第二輪觸發，GitHub 會直接取消第一輪
    的排隊、讓第二輪取代——等於兩輪互相取消，某些晚上反而只跑成一次。`queue: max`
    （2026-05 發布）讓同 group 的 run 改成 FIFO 真正排隊，已用手動連續 dispatch
    實測驗證有效（見 [PDCA.md](PDCA.md)）。第二輪不需要新的檢查邏輯——
    `has_equivalent_schedule` 本身就是冪等檢查，第一輪已成功時第二輪直接判定
    已存在並跳過，不會重複建立。
