...
> **00:20–06:00**。因為沒有後備了，同日加了**每晚兩輪排程**（15:30 + 16:00）——
...
3. 若**明天**尚無相同時段預約、且充電樁狀態為「充電樁已就緒」，就自動建立
   （目前為離峰 00:20–06:00；時段由 Variables 設定）。

「前一天觸發、目標日是明天」是刻意設計：即使排程延遲把實際執行時間推過午夜，
`target_date` 是每次執行當下重新算出的「明天」，不會做出「時段已過」的預約。
第一輪 15:30 觸發保留約 8h50m 緩衝（2026-09-03 從 22:05→20:30 再提早到 15:30，
因實測排程延遲達 3.5–5.8 小時，且希望第二輪失敗時也有真正的恢復空間，不被同樣
延遲一起壓縮到窗口邊緣，見 [docs/PDCA.md](docs/PDCA.md)）；**第二輪 16:00
是同日加的備援**——原自管主機停用後雲端版沒有後備了，第二輪給 reCAPTCHA／
裝置就緒狀態多一次機會。第二輪不會重複建立：如果第一輪已成功，它會直接判定
「已存在」並跳過（同一套冪等判斷）；如果第一輪失敗，就是完整重跑一次登入流程。

> ⚠️ **15:30 是台北時間下午**，比原本的 22:05/20:30 早很多。若你的車輛通常要
> 更晚才到家插上充電，觸發當下裝置可能還沒就緒，會被判定 `device_not_ready`
> 而跳過（正常綠燈，但不會建立預約）。若近期常看到這個情況，代表觸發時間比
> 實際回家時間早太多，需要往後調整（見 [docs/design.md](docs/design.md) §4 第 1 條）。
...
```bash
gh variable set INNOKNIGHT_START_TIME --body "00:20"
...
`daily-schedule.yml` 的 `schedule` cron **已於 Phase 4 啟用**，每天台北時間前一天
下午 15:30 與 16:00 各自動執行一次、建立隔日預約，排程觸發一律是正式寫入。要暫停
...
- 原自管主機已停用，現在只有雲端版一個系統在建立 00:20–06:00 的預約；
  要確認有沒有正常運作，直接看 Actions 頁的 run log。
...
| **Phase 4 啟用每晚自動排程** | ✅ 已啟用（15:30+16:00 兩輪）；辨識期已結束、已改回離峰 00:20–06:00 |
